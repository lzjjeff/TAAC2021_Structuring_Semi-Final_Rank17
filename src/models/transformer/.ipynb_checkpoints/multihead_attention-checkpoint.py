...
class MultiHeadAttentionFromPretrained(MultiHeadAttention):
    def __init__(self, hidden_size, head_size, n_heads):
        super(MultiHeadAttentionFromPretrained, self).__init__(hidden_size, head_size, n_heads)

    def forward(self, q, k, v, attn_mask=None, attn_weights=None):
        # q: [batch_size x len_q x d_model], k: [batch_size x len_k x d_model], v: [batch_size x len_k x d_model]
        batch_size = q.size(0)
        # (B, S, D) -proj-> (B, S, D) -split-> (B, S, H, W) -trans-> (B, H, S, W)
        q_s = self.W_q(q).view(batch_size, -1, self.n_heads, self.head_size).transpose(1,2)  # q_s: [batch_size x n_heads x len_q x d_k]
